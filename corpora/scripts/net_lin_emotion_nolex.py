...
def train(train_x, train_y):
	...
	# save network
	torch.save(lin_net.state_dict(), "net_lin_emotion_nolex.pt")
...
def make_data(list_of_datasets): 
	...
	for index, row in train_data.iterrows():
		...
		train_x.append([row["word_count"], row["upper_word_count"], row["ent_word_count"], row["cons_punct_count"]])
	...
	for index, row in test_data.iterrows():
		...
		test_x.append([row["word_count"], row["upper_word_count"], row["ent_word_count"], row["cons_punct_count"]])
...
train_x, test_x, train_y, test_y = make_data(["emotion_classification_1_clean.csv"])
lin_net = Lin_Net()
