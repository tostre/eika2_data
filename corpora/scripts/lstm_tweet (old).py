...
class Lstm(nn.Module):
	def __init__(self, vocab_size, output_size, embedding_dim, hidden_dim, n_layers, is_cuda, drop_prob=0.5):
		...
		self.lin1 = nn.Linear(hidden_dim * 85, hidden_dim) 		
	...
	def forward(self, x, hidden):	
		...
		print("out", out.shape)
		out = self.lin1(out.reshape(50, 85*256)) 
		out = self.lin2(out)
		return out, hidden
...
def load_datasets(list_of_datasets): 
	...
	# same for theis pos-datasets, feature vec, dim 1 = sequences, dim 2 = encodings for the elements in the sequence
	for f in list_of_datasets:
	...
	for index, line  in enumerate(data): 
		data[index] = [pos_encoding.get(entry, 18) for entry in line]
